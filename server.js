...
const validateName = (name) => {
  ...
  return nameRegex.test(name) && name.trim().length > 0 && !/^\s*$/.test(name) && !/[^a-zA-Z\s'-]/.test(name);
};
...
const validateComments = (comments) => {
  ...
  return commentsRegex.test(comments) || comments.trim() === '';
};
...
app.post('/api/submit', async (req, res) => {
  try {
    ...
    if (!validateName(name)) {
      return res.status(400).json({ error: 'Invalid name! Use only letters, spaces, hyphens, or apostrophes (e.g., "John Doe").' });
    }
    if (!validateEmail(email)) {
      return res.status(400).json({ error: 'Invalid email! Use a valid format (e.g., "name@domain.com").' });
    }
    if (!validatePhone(phone)) {
      return res.status(400).json({ error: 'Invalid phone! Use the format "123-456-7890".' });
    }
    if (!validateComments(comments)) {
      return res.status(400).json({ error: 'Invalid comments! Use only letters, numbers, spaces, or common punctuation (e.g., periods, commas).' });
    }
...
app.get('/results', async (req, res) => {
  try {
    ...
    const possibleMVPs = [
      ...
      "Jalen Green", "Jabari Smith Jr.", "Kawhi Leonard", "Paul George", "Stephen Curry"
    ];
    ...
    const scoredPredictions = predictions.map(prediction => {
      ...
      scores.finals.push({
        ...
        predGames,
        predMVP,
        predScore: predScore.join(' - '),
        claveWinner,
        claveGames,
        claveMVP,
        claveScore: claveScore.join(' - '),
        winnerMatch: winnerMatch ? 'Yes' : 'No',
        gamesMatch: gamesMatch ? 'Yes' : 'No',
        mvpMatch: mvpMatch ? 'Yes' : 'No',
        scoreDiff1: scoreDiff1,
        scoreDiff2: scoreDiff2,
        points
      });
